...
def Train(trainset,train_labels):
    prior_probability = np.zeros(class_num)                         # 先验概率  类别维度
    conditional_probability = np.zeros((class_num,feature_len,2))   # 条件概率  （类别维度，特征维度，特征取值范围（这里将图片二值化，所以是两个值））
...
if __name__ == '__main__':
    ...
    train_features, test_features, train_labels, test_labels = train_test_split(imgs, labels, test_size=0.2)
